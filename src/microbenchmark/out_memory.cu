...
void test_benchmark_graph_challenge(
    ...
    Safe_Call(cudaMalloc((void**)&(index_d_1), sizeof(float) * max_access_size));
    Safe_Call(cudaMemcpy(index_d_1, index[0], sizeof(float) * row_access[0].size(), cudaMemcpyHostToDevice));
    Safe_Call(cudaMalloc((void**)&(index_d_2), sizeof(float) * max_access_size));
    Safe_Call(cudaMemcpy(index_d_2, index[1], sizeof(float) * row_access[1].size(), cudaMemcpyHostToDevice));
