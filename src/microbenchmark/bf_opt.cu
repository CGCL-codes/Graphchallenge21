...
__global__ void bf_spmm(
  ...
  for(int i = 0; i < N_SLAB; i++) {
    ...
    __syncthreads();
    int no_name_len_beg = no_name_len[i] + threadIdx.y;
    int no_name_len_end = no_name_len[i + 1];
    for(int j = no_name_len_beg; j < no_name_len_end; j += blockDim.y) {
        int real_j = no_name_idx[j]
      float valY = Y0[rid * neuron + real_j];
    //   if(valY == 0) {
    //     continue;
    //   }

      int begOffW = roffW[i * neuron + real_j] + threadIdx.x;
      int endOffW = roffW[i * neuron + real_j + 1];
