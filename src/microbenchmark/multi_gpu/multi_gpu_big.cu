...
void test_benchmark_multi_gpu_graph_challenge(
    ...
    std::string kernel_event = "gpu_" + std::to_string(gpu_index) + "_kernel";   
    std::string memory_event = "gpu_" + std::to_string(gpu_index) + "_memory";   
    ...
    long batch_begin = (long)batch_index * (long)batch;
    long batch_end = std::min(((long)batch_index + 1) * (long)batch, (long)input.size());

    // printf("cpu2 b=36800, val idx 709, val = %f\n", input[36800][709]);
    for(long l = batch_begin; l < batch_end; ++l) {
        for(long i = 0; i < input[l].size(); ++i) {
            A[(long)(l - batch_begin) * (long)neuron + (long)i] = input[l][i];
        }
    }
	...
	std::cout << "[GPU "<< gpu_index << "]" << "Kernel Exec Time [20-our] = " << all_time <<  "ms" <<std::endl;
