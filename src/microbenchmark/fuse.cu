...
__global__ void fuseed_layer_l0_l11(
    ...
    for(int n = threadIdx.x; n < load_num[0] * MINIBATCH; n += blockDim.x){
        ...
        shared[f * max_shared_size + k] = A[categories[(blockIdx.x * MINIBATCH + f)] * neuron + (a_k) % neuron];
    }

    for(int layer = 0; layer < FUSE_LAYER; ++layer) {
        __syncthreads();
        // float* shared_use = &shared[layer * MINIBATCH * max_shared_size];
        ...
        for(int r = 0; r < 32; ++r) { // in graph challenge, all column has 32 nnzs
            ...
            for(int f = 0; f < MINIBATCH / UNROLL; ++f) {
                res[0 + f * UNROLL] += shared[(f * UNROLL + 0) * max_shared_size + idx] * val;
                res[1 + f * UNROLL] += shared[(f * UNROLL + 1) * max_shared_size + idx] * val;
                res[2 + f * UNROLL] += shared[(f * UNROLL + 2) * max_shared_size + idx] * val;
                res[3 + f * UNROLL] += shared[(f * UNROLL + 3) * max_shared_size + idx] * val;
                // res[4 + f * UNROLL] += shared[(f * UNROLL + 4) * max_shared_size + idx] * val;
                // res[5 + f * UNROLL] += shared[(f * UNROLL + 5) * max_shared_size + idx] * val;
                // res[6 + f * UNROLL] += shared[(f * UNROLL + 6) * max_shared_size + idx] * val;
                // res[7 + f * UNROLL] += shared[(f * UNROLL + 7) * max_shared_size + idx] * val;
            }
        }
        ...
        if(layer != FUSE_LAYER - 1) { // not last layer, write back to shared memory
            if(threadIdx.x < thread_num[layer])
                for(int f = 0; f < MINIBATCH ; ++f) {
                    shared[f * max_shared_size + threadIdx.x] = __ReLU(res[f] + bias);
                }
        }
        
        if(layer == FUSE_LAYER - 1) {
            if(threadIdx.x >= thread_num[layer] || blockIdx.y * thread_num[layer] + threadIdx.x >= neuron) {
...
void test_benchmark_fused_layer1024_0_1(
    ...
    fuseed_layer_l0_l11<<<grid, block, sizeof(float) * (MINIBATCH * shared_size_max), stream>>>(
        ...
        load_num_d, shared_size_d, thread_num_d, llll,
